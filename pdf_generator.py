...
class _CategorizedListFlow:
    ...
    def draw_category(self, category):
        ...
        # A category can opt into "title_style": true to render as a big,
        # centered, page-title-scale heading instead of the normal small
        # left-aligned category line -- used e.g. for "VIn ROSES"/"MAGnUMS"
        # section headers partway down a page, so they read as titles even
        # though the rest of that page's categories (country names) stay in
        # the normal left-aligned style. Uses the same display routine and
        # size as the actual page title (_new_page), with gap_after_page_title
        # as breathing room both above and below for that title-like weight.
        if category.get("title_style"):
            gap = cfg.get("gap_after_page_title", cfg["gap_after_category"])
            self.ensure_space(gap + cfg["page_title_size"] + gap)
            self.y -= gap
            cat_name = _apply_casing(category.get("name", ""), cfg.get("category_uppercase", True))
            self.y = _draw_display_aligned(
                self.c, cat_name, self.display_font_path, cfg["page_title_size"],
                self.col_left, self.col_right, self.y, "center",
            )
            self.y -= gap
