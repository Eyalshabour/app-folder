...
if sample:
    ...
    check("French accents survive into the PDF",
          "fumé" in text and "arménien" in text,
...
pairing = rendered.get("pairing_monde_voyage/en")
if pairing:
...
m = client.get("/api/menu?template=pairing_monde_voyage&language=en").get_json()["menu"]
target = m["courses"][0]["name"]
m["courses"][0]["wine"] = "ROUNDTRIP TEST 2020"
client.post("/api/save?template=pairing_monde_voyage&language=en", json=m)
back = client.get("/api/menu?template=pairing_monde_voyage&language=en").get_json()["menu"]
check("wine saves and reloads", back["courses"][0].get("wine") == "ROUNDTRIP TEST 2020")

pdf = os.path.join("generated_pdfs", "PairingMondeVoyage_EN_" +
...
client.post("/api/save?template=pairing_monde_voyage&language=en", json=back)
final = client.get("/api/menu?template=pairing_monde_voyage&language=en").get_json()["menu"]
